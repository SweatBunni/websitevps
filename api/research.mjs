...
export default async function handler(request) {
  ...
  try {
    ...
    if (kind === 'build') {
      if (!loader || !version) {
        return jsonResponse({ message: 'loader and version are required for build research.' }, { status: 400 });
      }
    ...
    return jsonResponse({ message: 'Unsupported research kind.' }, { status: 400 });
  } catch (error) {
    return jsonResponse({ message: error?.message || 'Research lookup failed.' }, { status: 500 });
  }
}
